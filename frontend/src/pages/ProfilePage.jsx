...
export default function ProfilePage() {
  ...
  return (
    /* Outer wrapper — normal flow, no position:fixed shader */
    <div style={{ background: 'var(--bg-black)', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '420px', zIndex: 0, pointerEvents: 'none' }}>
        ...
        {/* Fade shader into page bg */}
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom, rgba(17,24,39,0.45) 0%, rgba(17,24,39,1) 100%)' }} />
      </div>
      ...
      <div style={{ position:'relative', zIndex:1, paddingTop:100, paddingBottom:80 }}>
        <div className="container">
          ...
          <div style={{ display:'flex', alignItems:'center', gap:20, marginBottom:40, flexWrap:'wrap' }}>
            ...
            <div>
              <h1 style={{ fontFamily:"'Poppins',sans-serif", fontSize:'clamp(20px,4vw,34px)', fontWeight:800, marginBottom:4, color:'white' }}>{user?.name}</h1>
              <p style={{ color:'rgba(255,255,255,0.45)', margin:0, fontSize:14, fontFamily:"'Poppins',sans-serif" }}>
          ...
          <div className="row g-4">
            ...
            <div className="col-lg-3 col-md-4 col-12">
              <div style={{ background:'rgba(255,255,255,0.04)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:8 }}>
                {tabs.map(({ id, label, icon:Icon }) => (
                  <button key={id} onClick={() => setTab(id)} style={{
                    width:'100%', padding:'12px 16px',
                    background: tab===id ? 'rgba(249,115,22,0.12)' : 'transparent',
                    border:'none', borderRadius:10,
                    color: tab===id ? '#F97316' : 'rgba(255,255,255,0.55)',
            ...
            <div className="col-lg-9 col-md-8 col-12">
              <div style={{ background:'rgba(255,255,255,0.04)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:'clamp(18px,4vw,32px)' }}>

                {/* ── Profile tab ── */}
                {tab==='profile' && (
                  <>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:26, flexWrap:'wrap', gap:10 }}>
                      <h4 style={{ fontFamily:"'Poppins',sans-serif", fontWeight:700, margin:0, fontSize:'clamp(16px,3vw,20px)' }}>Personal Information</h4>
                    ...
                    <div className="row g-3">
                      {[{label:'Full Name',field:'name',Icon:FiUser,type:'text'},{label:'Phone',field:'phone',Icon:FiPhone,type:'tel'}].map(({label,field,Icon,type})=>(
                        <div key={field} className="col-md-6 col-12">
                          <label style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:'rgba(255,255,255,0.45)', textTransform:'uppercase', display:'block', marginBottom:6, letterSpacing:0.5 }}>{label}</label>
                          <div style={{ position:'relative' }}>
                            <Icon style={{ position:'absolute', left:13, top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', pointerEvents:'none' }} size={14}/>
                            <input type={type} value={form[field]} onChange={e=>setForm(f=>({...f,[field]:e.target.value}))} disabled={!editing}
                              ...
                              onFocus={e=>{e.target.style.borderColor='rgba(249,115,22,0.6)';e.target.style.background='rgba(249,115,22,0.06)';}}
                      ...
                      <div className="col-12">
                        <label style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:'rgba(255,255,255,0.45)', textTransform:'uppercase', display:'block', marginBottom:6, letterSpacing:0.5 }}>
                      ...
                      <div className="col-md-6 col-12">
                        <label style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:'rgba(255,255,255,0.45)', textTransform:'uppercase', display:'block', marginBottom:6, letterSpacing:0.5 }}>Email</label>
                        <div style={{ position:'relative' }}>
                          <FiMail style={{ position:'absolute', left:13, top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', pointerEvents:'none' }} size={14}/>
                          <input value={user?.email} disabled style={{ ...inputBase, opacity:0.4 }}/>
                        </div>
                        <p style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:4, fontFamily:"'Poppins',sans-serif" }}>Email cannot be changed</p>
                      </div>
                    </div>

                    {editing && (
                      <button onClick={()=>setEditing(false)} style={{ marginTop:16, background:'transparent', border:'1px solid rgba(255,255,255,0.15)', borderRadius:10, padding:'9px 20px', color:'rgba(255,255,255,0.55)', fontFamily:"'Poppins',sans-serif", fontWeight:600, fontSize:13, cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
                        <FiX size={13}/> Cancel
                      </button>
                    )}
                  </>
                ...
                {tab==='orders' && (
                  <>
                    <h4 style={{ fontFamily:"'Poppins',sans-serif", fontWeight:700, marginBottom:24, fontSize:'clamp(16px,3vw,20px)' }}>My Orders</h4>
                    {ordersLoading
                      ? <div className="text-center py-5"><div className="loader mx-auto"/></div>
                      : orders.length===0
                        ? <div style={{ textAlign:'center', padding:'48px 20px' }}>
                            <div style={{ fontSize:52, marginBottom:16 }}>📦</div>
                            <h5 style={{ fontFamily:"'Poppins',sans-serif", color:'rgba(255,255,255,0.6)', marginBottom:8 }}>No orders yet</h5>
                            <p style={{ color:'rgba(255,255,255,0.35)', fontSize:14 }}>Your order history will appear here</p>
                          </div>
                        : <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                            {orders.map(order=>(
                              <div key={order._id} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:'16px 20px' }}>
                                <div style={{ display:'flex', flexWrap:'wrap', justifyContent:'space-between', alignItems:'center', gap:10 }}>
                                  <div>
                                    <p style={{ margin:'0 0 4px', fontFamily:"'Poppins',sans-serif", fontWeight:700, fontSize:14, color:'white' }}>Order #{order._id.slice(-8).toUpperCase()}</p>
                                    <p style={{ margin:0, color:'rgba(255,255,255,0.4)', fontSize:12 }}>{new Date(order.createdAt).toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}</p>
                                  </div>
                ...
                {tab==='security' && (
                  <>
                    <h4 style={{ fontFamily:"'Poppins',sans-serif", fontWeight:700, marginBottom:24, fontSize:'clamp(16px,3vw,20px)' }}>Change Password</h4>
                    <form onSubmit={changePassword} style={{ maxWidth:420 }}>
                      {[{label:'Current Password',field:'currentPassword'},{label:'New Password',field:'newPassword'},{label:'Confirm New Password',field:'confirm'}].map(({label,field})=>(
                        <div key={field} style={{ marginBottom:16 }}>
                          <label style={{ fontFamily:"'Poppins',sans-serif", fontSize:11, color:'rgba(255,255,255,0.45)', textTransform:'uppercase', display:'block', marginBottom:6, letterSpacing:0.5 }}>{label}</label>
                          <div style={{ position:'relative' }}>
                            <FiLock style={{ position:'absolute', left:13, top:'50%', transform:'translateY(-50%)', color:'rgba(255,255,255,0.3)', pointerEvents:'none' }} size={14}/>
